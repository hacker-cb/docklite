...
class ProjectExistsError(AlreadyExistsError):
    ...
    def __init__(self, domain: str = None):
        message = f"Project with domain '{domain}' already exists" if domain else "Project already exists"
        super().__init__(message=message)
...
class InvalidComposeError(ValidationError):
    ...
    def __init__(self, details: str = None):
        message = f"Invalid docker-compose.yml: {details}" if details else "Invalid docker-compose.yml content"
